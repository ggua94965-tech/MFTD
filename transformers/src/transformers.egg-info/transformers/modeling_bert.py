...
class BertForACEBothOneDropoutSub(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.posi= AFT_FULL(config.hidden_size * 2, 32)
        self.qia = MUSEAttention(d_model=config.hidden_size * 2, d_k=config.hidden_size * 2, d_v=config.hidden_size * 2, h=8)
        ...
        self.feat_rescale = nn.Linear(int(self.hidden_dim / 2), self.hidden_dim)
        self.token_proj = nn.Linear(config.hidden_size, self.hidden_dim)
        ...
        self.discriminator = nn.Linear(config.hidden_size * 2, 2)
    ...
    def forward(
        ...
        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)
        feature_vector1 = feature_vector
        ...
        z_trans = self.feat_rescale(z_trans_raw).view(bsz, ent_len, self.hidden_dim)
